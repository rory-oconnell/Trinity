...
int double_hash(char* s){
    int hash = 0;
    int increment = 0;
	while(hashTable[hash] != NULL)
	{
		if (strcmp(hashTable[hash]->person, s) == 0){return hash;}
    	hash = hash_function(s) + (increment)*(hash3(s)) % ARRAY_SIZE;
        increment++;
	}
    return hash;
}
...
Element* createNewElement(char* name){
		int tableIndex = double_hash(name);
...
void addOrIncrement(char* name){
    ...
    // int tableIndex = hash_function(name);
	// hashTable[tableIndex] = malloc(sizeof(Element));
	// hashTable[tableIndex]->person = malloc(sizeof(char)*MAX_STRING_SIZE);
	// strcpy(hashTable[tableIndex]->person, name);
...
int main()
{
	...
	if(load_return == 1){printf("File names.csv loaded!\n");}
	printf("Capacity: %d\n", ARRAY_SIZE);
	printf("Num Terms: %d\n", num_terms);
	printf("Collisions: %d\n", collisions);
