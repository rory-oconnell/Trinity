...
#include <stdio.h>
  ...
  //assuming name will not be longer than 49 characters, as we need one character for the null terminator
    ...
    else
    {
        ...
        while(!feof(pf)) //continue to read the file so long as we have not(!) reached the end of the file
        {
            ...
            if (exclude_first_line_counter < 1)
            {
                ...
                {
                    if (attribute_index == 13 && ch_as_int == 10)   //this will be the code that enacts when the newline is found
                    {
                    ...
                    if (attribute_index == 13)
                    {
                            FEEDER[feeder_count] = ch;
                            feeder_count++;
                    }

                    else if (ch_as_int != 44 && ch_as_int != 10)    // if the read character does not equal a comma, pass it to the array at position feeder_count
                        {
                            FEEDER[feeder_count] = ch;
                            feeder_count++;
                        }

                    else if (ch_as_int == 44 && attribute_index != 13)   //when we encounter a comma, the following series of if else statments takes over
                        {
                            FEEDER[feeder_count] = ch;
                            feeder_count++;
